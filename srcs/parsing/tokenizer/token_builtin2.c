...
/*   Updated: 2024/08/11 22:38:18 by jewu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//it seems exit arg works (just one arg), so handle only number or arguments too?
...
int export_ok(t_shell *gear_5, t_token *token, t_env *envp)
{
    ...
    else
    {
        while (arg)
        {
            if (arg->token_type != TOKEN_VARIABLEASSIGNATION && arg->token_type != TOKEN_ARG)
            {
                gear_5->exit_status = 1;
